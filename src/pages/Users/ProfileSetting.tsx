...
interface UpdateProfileProps {
    userData: any; // fetched from backend (getMyProfile query)
}

export default function ProfileSetting({ userData }: UpdateProfileProps) {
    ...
    const handleSubmitProfile = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const updatedUser = await updateUser(form).unwrap();
            toast.success("Profile updated successfully!");
            setForm({ name: updatedUser.name || "", phone: updatedUser.phone || "" });
            setProfileUpdated(true);
            setTimeout(() => setProfileUpdated(false), 2000); // animation reset
        } catch (err: any) {
            console.error(err);
    ...
    return (
        <motion.div
            className="flex flex-col items-center p-4 sm:p-6 bg-gray-50 min-h-screen gap-8"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
                ...
                <AnimatePresence>
                    {profileUpdated && (
                        <motion.div
                            ...
                            exit={{ opacity: 0, y: -10 }}
                            className="text-green-600 font-medium text-center mb-2"
                        >
                            Profile Updated!
                        </motion.div>
                    )}
